    ...
    <div className="space-y-4">
      ...
      {active.length === 0 && !showForm ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {active.map((p) => (
            <ProjectCard key={p.id} project={p} totals={getProjectTotals(p.id)} onUpdate={updateProject} onDelete={deleteProject} />
          ))}
        </div>
      )}
...
function ProjectCard({ project, totals, onUpdate, onDelete }: {
  ...
  return (
    <div className={`bg-bg-card border rounded-lg p-4 space-y-3 ${isArchived ? 'border-border/40 opacity-60' : 'border-border'}`}>
      {/* Header */}
          ...
          {editing ? (
            ...
            <>
              ...
              <button
                ...
                title={isArchived ? 'Reativar' : 'Encerrar'}
              >
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-text-secondary">{totals.count} transacoes</span>
      </div>
